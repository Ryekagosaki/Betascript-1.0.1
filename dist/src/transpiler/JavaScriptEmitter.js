...
class JavaScriptEmitter {
    ...
    emitClassMember(member) {
        if (member.type === "MethodDeclaration") {
            ...
            if (member.kind === "method") {
                const isAsync = member.body.statements.some(s => s.type === "FunctionDeclaration" && s.isAsync) || member.body.statements.some(s => s.type === "ExpressionStatement" &&
    ...
    emitExportStatement(stmt) {
        if (stmt.declaration.type === "FunctionDeclaration") {
            ...
            if (decl.superclass)
                classStr += " extends " + decl.superclass;
            this.emitLine(classStr + " {");
            this.indent++;
        ...
        switch (expr.type) {
            ...
            case "Literal":
                ...
                if (expr.value === undefined)
                    return "undefined";
                if (typeof expr.value === "string" && (expr.raw.startsWith("`") || expr.raw.startsWith("/")))
                    return expr.raw;
            ...
            case "ObjectExpression":
                const props = expr.properties.map(p => {
                    if (p.key === "__spread")
                        return `...${this.emitExpression(p.value)}`;
    ...
    emitMemberExpression(expr) {
        ...
        const prop = expr.computed
            ...
            : `${expr.optional ? "?." : "."}${expr.property.name}`;
        return `${obj}${prop}`;
    }
    emitLambdaBody(body) {
        const statements = body.statements.map(s => {
            if (s.type === "ReturnStatement") {
                if (s.argument)
                    return `return ${this.emitExpression(s.argument)};`;
                return "return;";
            }
